...
{
    public partial class 出库申请单明细 : CustomFlowForm
    {
        /// <summary>
        /// 错误信息
        /// </summary>
        ...
        /// 单据号
        /// </summary>
        private Business_WarehouseOutPut_Requisition m_lnqBillInfo = new Business_WarehouseOutPut_Requisition();

        public Business_WarehouseOutPut_Requisition LnqBillInfo
        {
        ...
        /// 明细视图数据集合
        /// </summary>
        List<View_Business_WarehouseOutPut_RequisitionDetail> m_listViewDetail = new List<View_Business_WarehouseOutPut_RequisitionDetail>();
        ...
        public override void LoadFormInfo()
        {
            try
            {
                m_billNoControl = new BillNumberControl(CE_BillTypeEnum.出库申请单.ToString(), m_serviceRequistion);
        ...
        void 关联业务_m_OnCompleteSearch()
        {
            ...
            if (drTemp != null)
            {
                ...
                {
                    //case OutPutBusinessType.生产采购:
                    //case OutPutBusinessType.普通采购:
                    //case OutPutBusinessType.样品采购:
        ...
        void SetInfo()
        {
            ...
            else
            {
                ...
                m_lnqBillInfo = new Business_WarehouseOutPut_Requisition();
        ...
        void RefreshDataGridView(List<View_Business_WarehouseOutPut_RequisitionDetail> source)
        {
            if (source != null)
            {
                customDataGridView1.Rows.Clear();
                foreach (View_Business_WarehouseOutPut_RequisitionDetail item in source)
                {
            ...
            try
            {
                if (cmbBillType.Text.Trim().Length > 0)
                {
                    switch (GlobalObject.GeneralFunction.StringConvertToEnum<CE_OutPutBusinessType>(cmbBillType.Text))
                    {
                        ...
                        //    frm.BlDateTimeControlShow = false;

                        //    if (frm.ShowDialog() == DialogResult.OK)
                        //    {
                        //        List<string> listTemp = DataSetHelper.ColumnsToList(frm.DtResult, "订单号");
        ...
        private bool customForm_PanelGetDateInfo(CE_FlowOperationType flowOperationType)
        {
            try
            {
                ...
                List<View_Business_WarehouseOutPut_RequisitionDetail> listTemp = new List<View_Business_WarehouseOutPut_RequisitionDetail>();

                foreach (DataGridViewRow dgvr in customDataGridView1.Rows)
                {
                    View_Business_WarehouseOutPut_RequisitionDetail detailTemp = new View_Business_WarehouseOutPut_RequisitionDetail();
