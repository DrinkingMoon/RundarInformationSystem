    ...
    internal class ExpressionParser
    {
        ...
        void ProcessParameters(ParameterExpression[] parameters) {
            foreach (ParameterExpression pe in parameters)
                if (!GlobalObject.GeneralFunction.IsNullOrEmpty(pe.Name))
                    AddSymbol(pe.Name, pe);
            if (parameters.Length == 1 && GlobalObject.GeneralFunction.IsNullOrEmpty(parameters[0].Name))
                it = parameters[0];
        }
