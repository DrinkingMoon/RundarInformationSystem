...
namespace GlobalObject
{
    ...
    /// </summary>
    public class MultiLevelXmlParams
    {
        /// <summary>
        /// 根节点名称
        /// </summary>
        ...
        public string FileName
        {
            ...
            set
            {
                Debug.Assert(!GlobalObject.GeneralFunction.IsNullOrEmpty(value), "文件名不能为空或空串");
                m_xmlFileName = value;
            }
        }
        ...
        public MultiLevelXmlParams(string xmlFileName)
        {
        ...
        public Dictionary<string, Object> GetParams()
        {
            ...
            {
                param.Add(childNode.Attributes[0].Value, GetParam(childNode));
            }
        ...
        Dictionary<string, object> GetParam(XmlNode parentNode)
        {
            ...
            foreach (XmlNode childNode in parentNode.ChildNodes)
            {
                ...
                {
                    attribute.Add(attr.Name, attr.Value);
                }

                paramItem.Add(childNode.InnerText, attribute);
            }
        ...
        void SaveNewParams(Dictionary<string, Object> dicParams)
        {
            ...
            foreach (KeyValuePair<string, Object> param in dicParams)
            {
                ...
                Dictionary<string, object> dicChildParam = param.Value as Dictionary<string, object>;
            ...
            if (dicParams != null)
            {
                foreach (KeyValuePair<string, Object> param in dicParams)
                {
                    ...
                    else
                    {
                        ...
                        foreach (KeyValuePair<string, object> paramItem in dicChildParam)
                        {
                            bFind = false;
                            ...
                            foreach (XmlNode node in dataNode.ChildNodes)
                            {
                                if (paramItem.Key.CompareTo(node.InnerText) == 0)
                                {
                                    ...
                                    Dictionary<string, string> attributes = paramItem.Value as Dictionary<string, string>;

                                    foreach (KeyValuePair<string, string> attr in attributes)
                                    {
                                        node.Attributes[attr.Key].Value = attr.Value;
                                    }
                                    break;
                                }
                            }
        ...
        void SaveParam(XmlDocument doc, XmlNode parentNode, Dictionary<string, object> paramItems)
        {
            foreach (KeyValuePair<string, object> item in paramItems)
            {
        ...
        void SaveParam(XmlDocument doc, XmlNode parentNode, string paramName, object paramValue)
        {
            Dictionary<string, string> attributes = paramValue as Dictionary<string, string>;
            ...
            {
                XmlNode attr = doc.CreateNode(XmlNodeType.Attribute, item.Key, null);
