...
namespace Expression
{
    ...
    /// 自制件入库报表界面
    /// </summary>
    public partial class 报表_自制件入库单 : Form, IBillReportInfo
    {
        string m_billID = "";
        ...
        public 报表_自制件入库单(string billID, string billName)
        {
            InitializeComponent();
            m_billID = billID;
            m_billName = billName;
        ...
        private void InitData()
        {
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                string sql = "select * from dbo.View_S_HomemadePartBill where 入库单号='" + m_billID + "'";
                SqlCommand command = new SqlCommand(sql);
                command.Connection = conn;

                SqlDataAdapter adapter = new SqlDataAdapter(command);
                adapter.Fill(this.DepotManagementDataSet.View_S_HomemadePartBill);

                ReportDataSource rds = new ReportDataSource("DepotManagementDataSet_View_S_HomemadePartBill", 
                                                             this.DepotManagementDataSet.View_S_HomemadePartBill);
