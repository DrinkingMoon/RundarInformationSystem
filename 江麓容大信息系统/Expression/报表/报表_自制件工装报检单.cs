...
namespace Expression
{
    ...
    /// 普通入库报表界面
    /// </summary>
    public partial class 报表_自制件工装报检单 : Form, IBillReportInfo
    {
        string m_billID = "";
        ...
        public 报表_自制件工装报检单(string billID, string billName, bool needInitData)
        {
            InitializeComponent();
            m_billID = billID;
            m_billName = billName;
        ...
        private void InitData()
        {
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                string sql = "select * from dbo.View_S_FrockInDepotBill where 单据号='" + m_billID + "'";
                SqlCommand command = new SqlCommand(sql);
                command.Connection = conn;

                SqlDataAdapter adapter = new SqlDataAdapter(command);
                adapter.Fill(this.DepotManagementDataSet.View_S_FrockInDepotBill);

                ReportDataSource rds = new ReportDataSource("DepotManagementDataSet_View_S_FrockInDepotBill", 
                                                            this.DepotManagementDataSet.View_S_FrockInDepotBill);

                reportViewer1.LocalReport.DataSources.Clear();
                reportViewer1.LocalReport.DataSources.Add(rds);

                sql = "select * from DepotManagement.dbo.View_S_FrockInDepotGoodsBill where 单据号='" + m_billID + "'";
                command = new SqlCommand(sql);
                command.Connection = conn;

                adapter = new SqlDataAdapter(command);
                adapter.Fill(this.DepotManagementDataSet.View_S_FrockInDepotGoodsBill);

                rds = new ReportDataSource("DepotManagementDataSet_View_S_FrockInDepotGoodsBill",
                    this.DepotManagementDataSet.View_S_FrockInDepotGoodsBill);
        ...
        private void 报表_自制件工装报检单_Load(object sender, EventArgs e)
        {
        }
    }
}
