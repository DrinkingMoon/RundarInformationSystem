...
namespace Expression
{
    ...
    /// 领料物品详单
    /// </summary>
    public partial class FormFetchGoods2 : Form
    {
        #region 成员变量
        ...
        public FormFetchGoods2(OperateMode operateMode, string billNo, string reliantBillNo)
        {
            InitializeComponent();
            ...
            m_billInfo = GetBillView(m_billNo); //ServerModuleFactory.GetServerModule<IMaterialRequisitionServer>().GetBillView(m_billNo);
        ...
        private View_S_MaterialRequisition GetBillView(string billNo)
        {
            View_S_MaterialRequisition result = new View_S_MaterialRequisition();

            string sql = string.Format("select * from DepotManagement.dbo.View_S_MaterialRequisition_Temp where 领料单号 = '{0}'", billNo);

            DataTable dt = DatabaseServer.QueryInfo(sql);

            result.备注 = dt.Rows[0]["备注"].ToString();
            result.编制人 = dt.Rows[0]["编制人"].ToString();
            result.编制人编码 = dt.Rows[0]["编制人编码"].ToString();
            result.部门领导签名 = dt.Rows[0]["部门领导签名"].ToString();
            result.部门名称 = dt.Rows[0]["部门名称"].ToString();
            result.部门主管签名 = dt.Rows[0]["部门主管签名"].ToString();
            result.仓管签名 = dt.Rows[0]["仓管签名"].ToString();
            result.产品类型 = dt.Rows[0]["产品类型"].ToString();
            result.出库时间 = dt.Rows[0]["出库时间"] == null ? (DateTime?) null : Convert.ToDateTime(dt.Rows[0]["出库时间"].ToString());
            result.单据状态 = dt.Rows[0]["单据状态"].ToString();
            result.工艺人员签名 = dt.Rows[0]["工艺人员签名"].ToString();
            result.关联单号 = dt.Rows[0]["关联单号"].ToString();
            result.关联单据 = dt.Rows[0]["关联单据"].ToString();
            result.库房代码 = dt.Rows[0]["库房代码"].ToString();
            result.库房名称 = dt.Rows[0]["库房名称"].ToString();
            result.领料部门 = dt.Rows[0]["领料部门"].ToString();
            result.领料单号 = dt.Rows[0]["领料单号"].ToString();
            result.领料类型 = dt.Rows[0]["领料类型"].ToString();
            result.领料台数 = Convert.ToInt32(dt.Rows[0]["领料台数"].ToString());
            result.日期 = Convert.ToDateTime(dt.Rows[0]["日期"].ToString());
            result.是否已打印 = false;
            result.用途编码 = dt.Rows[0]["用途编码"].ToString();
            result.用途说明 = dt.Rows[0]["用途说明"].ToString();            

            return result;
        }

        private void FormFetchGoods2_Load(object sender, EventArgs e)
        {
            RefreshForm();
        ...
        void FormFetchGoods2_Resize(object sender, EventArgs e)
        {
        ...
        void RefreshForm()
        {
            ...
            RefreshDataGridView(GetGoods(m_billNo));
        }
        ...
        private DataTable GetGoods(string billNo)
        {
            string sql = string.Format("select * from DepotManagement.dbo.View_S_MaterialRequisitionGoods_Temp where 领料单号= '{0}' ", billNo);

            sql += " order by 图号型号, 物品名称, 显示位置";

            return DatabaseServer.QueryInfo(sql);
        }
        ...
        void RefreshDataGridView(DataTable dataSource)
        {
            if (dataSource == null)
            {
                return;
            ...
            dataGridView1.DataSource = dataSource;//GlobalObject.GeneralFunction.ConvertToDataTable<View_S_MaterialRequisitionGoods>(goods);
        ...
        private void FormFetchGoods2_Shown(object sender, EventArgs e)
        {
