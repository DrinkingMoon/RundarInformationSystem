 ...
 *
 * 文件名称:  UserControlFetchGoodsGatherBill.cs
...
namespace Expression
{
    ...
    /// 零件汇总报表组件
    /// </summary>
    public partial class 零部件领料汇总表 : Form
    {
        /// <summary>
        /// 查询结果
        /// </summary>
        IQueryable<View_S_FetchGoodsSummaryTable> m_findBill;
        ...
        /// 零部件领用汇总表
        /// </summary>
        报表_零部件领料汇总 m_reportAccessoryGatherBill;
        ...
        public 零部件领料汇总表(FunctionTreeNodeInfo nodeInfo)
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private void UserControlFetchGoodsGatherBill_Resize(object sender, EventArgs e)
        {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void UserControlFetchGoodsGatherBill_Load(object sender, EventArgs e)
        {
        ...
        bool ExecuteGather()
        {
            ...
            if (cmbType.Text == "零部件汇总")
            {
                ...
                {
                    m_findBill = m_detailSummaryInfo.GetFetchGoodsSummarInfo(
                        dtpStartTime.Value.Date, dtpEndTime.Value.Date, SummaryMode.图号);
                }
                else if (cmbOrder.Text == "按名称汇总")
                {
                    m_findBill = m_detailSummaryInfo.GetFetchGoodsSummarInfo(
                        dtpStartTime.Value.Date, dtpEndTime.Value.Date, SummaryMode.名称);
                }
                else if (cmbOrder.Text == "按材料类别汇总")
                {
                    m_findBill = m_detailSummaryInfo.GetFetchGoodsSummarInfo(
                        dtpStartTime.Value.Date, dtpEndTime.Value.Date, SummaryMode.材料类别);
                }
            }
            else
            {
        ...
        void RefreshDataGridView()
        {
            ...
            {
                dt = m_detailSummaryInfo.GetGetFetchGoodsSummarInfoForStorageID(dtpStartTime.Value.Date,
                    dtpEndTime.Value.Date,
                    UniversalFunction.GetStorageID(cmbStorage.Text));
            }
            else
            {
                dt = GlobalObject.GeneralFunction.ConvertToDataTable<View_S_FetchGoodsSummaryTable>(m_findBill);
            }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (ExecuteGather())
            {
                m_reportAccessoryGatherBill = new 报表_零部件领料汇总(
                    cmbOrder.SelectedItem.ToString(), dtpStartTime.Value.Date, dtpEndTime.Value.Date);
