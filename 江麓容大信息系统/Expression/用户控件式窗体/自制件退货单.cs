...
namespace Expression
{
    ...
    /// 自制件退货单界面
    /// </summary>
    public partial class 自制件退货单 : Form
    {
        /// <summary>
        /// 单据类消息发布器
        /// </summary>
        ...
        /// 单据服务组件
        /// </summary>
        IHomemadeRejectBill m_billServer = ServerModuleFactory.GetServerModule<IHomemadeRejectBill>();
        ...
        /// 服务
        /// </summary>
        IHomemadeRejectBill m_PdPlan = ServerModuleFactory.GetServerModule<IHomemadeRejectBill>();
        ...
        private void 自制件退货单_Load(object sender, EventArgs e)
        {
        ...
        void ClearForm()
        {
            ...
            //txtProvider.Text = ""; 自制件退货的唯一供应商 只有机加车间 Modify by cjb 2012.4.24
            txtReason.Text = "";
            txtDepartment.Text = "";
            txtRemark.Text = "";
            txtProposer.Text = "";
        ...
        private void 自制件退货单_Resize(object sender, EventArgs e)
        {
        ...
        protected override void DefWndProc(ref Message m)
        {
            switch (m.Msg)
            {
                ...
                case WndMsgSender.PositioningMsg:
                    ...
                    DataTable dtMessage = UniversalFunction.PositioningOneRecord(msg.MessageContent, "自制件退货单");
        ...
        private void ReturnBillStatus()
        {
            ...
            {
                回退单据 form = new 回退单据(CE_BillTypeEnum.自制件退货单, txtBill_ID.Text, lblBillStatus.Text);
                ...
                if (form.ShowDialog() == DialogResult.OK)
                {
                    if (MessageBox.Show("您确定要回退此单据吗？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        if (m_billServer.ReturnBill(form.StrBillID,
                            form.StrBillStatus, out m_queryResult, out m_strErr, form.Reason))
                        {
        ...
        void EndFlowMessage(string billNo, string msgContent)
        {
            ...
            #region 发送知会消息
            BillMessagePromulgatorServer promulgator = new BillMessagePromulgatorServer();

            string content = string.Format("{0} 号采购退货单已经处理完毕", msg.单据流水号);

            List<string> noticeRoles = new List<string>();

            noticeRoles.AddRange(m_billMessageServer.GetDeptDirectorRoleName(dataGridView1.SelectedRows[0].Cells["申请部门编码"].Value.ToString()));
            ...
            promulgator.NotifyMessage(msg.单据类型, msg.单据号, content, BasicInfo.LoginID, noticeRoles, null);
            #endregion 发送知会消息
        }
        ...
        private void 领料员提交单据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!m_billServer.SubmitNewBill(billNo, out m_queryResult, out m_strErr))
            {
                MessageDialog.ShowErrorMessage(m_strErr);
                return;
            }

            MessageDialog.ShowPromptMessage("成功提交,等待财务审核!");
        ...
        private void 删除单据ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            MaterialRejectBillBillStatus status =
                ...
                UniversalFunction.GetBillStatus("S_HomemadeRejectBill", "BillStatus", "Bill_ID", txtBill_ID.Text));
            ...
            if (!m_billServer.DeleteBill(billNo, out m_queryResult, out m_strErr))
            {
                MessageDialog.ShowErrorMessage(m_strErr);
                return;
            }
        ...
        private void 批准退货ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!m_billServer.FinanceAuthorizeBill(billNo, BasicInfo.LoginName, out m_queryResult, out m_strErr))
            {
                MessageDialog.ShowErrorMessage(m_strErr);
                return;
            }
            ...
            PassFlowMessage(billNo, string.Format("{0}号自制件退货单已提交，请仓管处理", billNo),
        ...
        private void 核实退货清单ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            FormHomemadeRejectlist form =
                new FormHomemadeRejectlist(FormHomemadeRejectlist.OperateMode.仓库核实, txtProvider.Text, txtBill_ID.Text);
            form.ShowDialog();
        }

        private void 确认退货ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!CheckSelectedRow())
                return;
            ...
            if (!m_billServer.FinishBill(txtBill_ID.Text, BasicInfo.LoginName, out m_queryResult, out m_strErr))
            {
                MessageDialog.ShowErrorMessage(m_strErr);
                return;
            }
            ...
            EndFlowMessage(billNo, string.Format("{0}号自制件退货单已完成", billNo));
            RefreshDataGridView(m_queryResult);
            PositioningRecord(billNo);
        }
        ...
        private void 表单打印ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            报表_自制件退货单 report = new 报表_自制件退货单(txtBill_ID.Text, labelTitle.Text);
            report.ShowDialog();
        ...
        {
            FormHomemadeRejectlist form = new FormHomemadeRejectlist(
                FormHomemadeRejectlist.OperateMode.查看, txtProvider.Text, txtBill_ID.Text);
            form.ShowDialog();
        }
    }
}