...
{
    public partial class 借货单 : Form
    {
        /// <summary>
        /// 借货服务组件
        /// </summary>
        IProductLendService m_serverLend = ServerModuleFactory.GetServerModule<IProductLendService>();
        ...
        void RefrshData()
        {
            m_dtSource = m_serverLend.GetBillInfo();
        ...
        protected override void DefWndProc(ref Message m)
        {
            switch (m.Msg)
            {
                ...
                case WndMsgSender.PositioningMsg:
                    ...
                    DataTable dtMessage = UniversalFunction.PositioningOneRecord(msg.MessageContent, CE_BillTypeEnum.借货单.ToString());
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageDialog.ShowEnquiryMessage("是否要删除【" + billNo + "】号单据?")
                == DialogResult.Yes)
            {
                if (m_serverLend.DeleteBill(billNo, out m_strError))
                {
        ...
        {
            借货单明细 frm = new 借货单明细(null);
            frm.ShowDialog();
            RefrshData();
            PositioningRecord(frm.LnqBill.BillNo);
        }

        private void 借货单_Load(object sender, EventArgs e)
        {
            RefrshData();
