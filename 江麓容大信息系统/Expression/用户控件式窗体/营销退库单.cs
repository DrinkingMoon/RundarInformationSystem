...
namespace Expression
{
    ...
    /// 营销退库单界面
    /// </summary>
    public partial class 营销退库单 : Form
    {
        /// <summary>
        /// 单据编号控制类
        /// </summary>
        ...
        /// 构造函数
        /// </summary>
        public 营销退库单(PlatformManagement.FunctionTreeNodeInfo nodeInfo)
        {
            InitializeComponent();

            m_billNoControl = new BillNumberControl(CE_BillTypeEnum.营销退库单, m_findSellIn);

            m_msgPromulgator.BillType = "营销退库单";
        ...
        /// <param name="e"></param>
        private void 营销退库_Load(object sender, EventArgs e)
        {
        ...
        {
            DataTable dt = m_findSellIn.GetAllBill(CE_MarketingType.退库.ToString(),
        ...
        private void ShowForm()
        {
            ...
            营销退库明细单 form = new 营销退库明细单(Convert.ToInt32(dgv_Show.CurrentRow.Cells["ID"].Value), m_authFlag);
            form.ShowDialog();
        ...
        private void 营销退库_Resize(object sender, EventArgs e)
        {
        ...
        private void btnReback_Click(object sender, EventArgs e)
        {
            ...
            {
                回退单据 form = new 回退单据(CE_BillTypeEnum.营销退库单, billNo,
                ...
                if (form.ShowDialog() == DialogResult.OK)
                {
                    if (MessageBox.Show("您确定要回退此单据吗？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        if (m_findSellIn.ReturnBill(form.StrBillID,
                            form.StrBillStatus, out m_err, form.Reason, "营销退库单"))
                        {
