...
{
    public partial class 车间耗用单 : Form
    {
        /// <summary>
        /// 车间耗用服务组件
        /// </summary>
        ...
        void RefrshData()
        {
            m_dtSource = m_serverConsumption.GetBillInfo();
        ...
        protected override void DefWndProc(ref Message m)
        {
            switch (m.Msg)
            {
                ...
                case WndMsgSender.PositioningMsg:
                    ...
                    DataTable dtMessage = UniversalFunction.PositioningOneRecord(msg.MessageContent, CE_BillTypeEnum.车间耗用单.ToString());
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageDialog.ShowEnquiryMessage("是否要删除【" + billNo + "】号单据?")
                == DialogResult.Yes)
            {
                if (m_serverConsumption.DeleteBill(billNo, out m_strError))
                {
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            车间耗用单明细 frm = new 车间耗用单明细(billNo);
            frm.ShowDialog();
            RefrshData();
        ...
        {
            车间耗用单明细 frm = new 车间耗用单明细(null);
            frm.ShowDialog();
            RefrshData();
            PositioningRecord(frm.StrBillNo);
        }

        private void 车间耗用单_Load(object sender, EventArgs e)
        {
            RefrshData();
